			...
			public void actionPerformed(ActionEvent e) {
				...
				if(bookfind) {
					try(BufferedWriter writer= new BufferedWriter(new FileWriter("issuedBooks.txt", true))){
						 ...
						 writer.write(bookid + "%" + bookname + "%" + userid + "%" + username + "%" + formattedIssueDate + "%" + formattedDueDate+ "%"+isBookReturned);
	                writer.newLine();
...
//							isBReturn= true;
				...
				if(bookfind) {
					try(Scanner readFile= new Scanner(new File("issuedBooks.txt"));
						...
						while(readFile.hasNextLine()) {
							...
							writer.write(bid+"%"+ bname+"%"+uid+"%"+isDate+"%"+duDate+"%"+isBReturn);
							writer.newLine();
	...
	void viewIssuedBook(){
		...
		String[] columnNames= {"Book Id", "Book Name", "User Id", "User Name", "Issued Date", "Due Date", "Return Book" };
		DefaultTableModel model= new DefaultTableModel();
		model.setColumnIdentifiers(columnNames);
